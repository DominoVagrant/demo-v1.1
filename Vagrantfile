...
#  config.vm.provision "shell", name: "Create names.nsf", inline: "sudo -E su - vagrant -c \"cd dist; java -jar CreateNamesDatabase.jar\"", sensitive:true, env: {"PASSWORD" => NOTES_ID_PASSWORD}, run:"always"
  ...
  config.vm.provision "shell", name: "Copy Genesis-0.6.16.jar to /local/dominodata/JavaAddin/Genesis", privileged:true, inline: "cp /home/vagrant/dist/Genesis-0.6.16.jar /local/dominodata/JavaAddin/Genesis; chown domino:domino /local/dominodata/JavaAddin/Genesis/Genesis-0.6.16.jar" #####, run:"always"
