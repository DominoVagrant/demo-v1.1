...
#  config.vm.provision "shell", name: "Create names.nsf", inline: "sudo -E su - vagrant -c \"cd dist; java -jar CreateNamesDatabase.jar\"", sensitive:true, env: {"PASSWORD" => NOTES_ID_PASSWORD}, run:"always"
  ...
  config.vm.provision "shell", name: "Copy new user ID to NotesJava", privileged:true, inline: "cp /local/dominodata/demo-user.id /local/notesjava/user.id; chown vagrant:vagrant /local/notesjava/user.id" 
  ...
  config.vm.provision "shell", name: "Copy Genesis-0.6.15.jar to /local/dominodata/JavaAddin/Genesis", privileged:true, inline: "cp /home/vagrant/dist/Genesis-0.6.15.jar /local/dominodata/JavaAddin/Genesis; chown domino:domino /local/dominodata/JavaAddin/Genesis/Genesis-0.6.15.jar" #####, run:"always"
